 ...
 * I N D E X
 * ====================================
 * 1) Plugin Function Calls - 23
 * 2) Initialize Variables - 34
 * 3) Set Initial Settings - 43
 * 4) Set iOS Conditions - 88
 * 5) Set Save and Load Slots - 98
 * 6) Save and Load Events - 136
 * 7) Settings Event Handlers - 221
 * 8) Data-Action Event Handlers - 265
 * 9) In-Game Event Handlers - 346
 * 10) Engine Helper Functions - 434
 * 11) Statements Functioning - 613
 * ====================================
**/
...
$(document).ready(function(){
	...
	**/
	...
	function setSlots(){
		for(var i = 1; i <= engine["Slots"]; i++){
			...
			var slot = $_.storage.get("Save_" + i);
			if(slot == null){
				$_.storage.set("Save_"+i,"");
			}else if(slot != ""){
	...
	**/
	...
	$("[data-menu='load']").on("click","[data-load-part]", function(){
		...
		var data = JSON.parse($_.storage.get("Save_" + $(this).data("load-slot")));
		engine = data["Engine"];
	...
	$("[data-menu='save']").on("click","[data-save]", function(){
		if(playing){
			...
			console.log(JSON.stringify(engine));
		     $_.storage.set("Save_" + $(this).data("save"), '{"Date":"' + day + "-"+month + "-" + year + '","Engine":' + JSON.stringify(engine) + ',"Show":"' + show + '","Label":"' + engine["Label"] + '"}');
				...
				switch(parts[0]){
					...
					default:
						...
						if(characters[parts[0]] != null){
							...
							document.querySelector("[data-ui='say']").innerHTML =parts.join(" ").replace(parts[0] + " ", "");
							...
							document.querySelector("[data-ui='say']").innerHTML = parts.join(" ");
						}
